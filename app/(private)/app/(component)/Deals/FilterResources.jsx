...
function FilterResources({ resourcesData, onFilterChange }) {
  ...
  // Update localStorage and notify parent when categoryFilter changes
  useEffect(() => {
    ...
    <Dropdown radius="none" shadow="lg">
      <DropdownTrigger>
        <div
          ...
          className="sticky bg-white bottom-0 h-[95px] [aria-expanded='true']:bg-blue-500"
        >
          <div className="px-2 py-2 text-[14px] xl:text-lg font-medium text-center border-3 lg:py-6 lg:px-6 border-color5 text-color02 shadow-lg flex items-center gap-5 cursor-pointer hover:bg-color5 hover:text-white group transition-colors duration-200">
            Filter Resources
            <svg
      ...
      >
        <DropdownItem
          color="danger"
          onPress={() => setCategoryFilter(new Set(uniqueCategories))}
